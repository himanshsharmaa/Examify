...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $password_hash = !empty($password) ? password_hash($password, PASSWORD_BCRYPT) : $student['password'];
    ...
    if (empty($name) || empty($email)) {
        ...
        $stmt = $db->prepare($query);
        $stmt->bind_param('sssd', $name, $email, $password_hash, $student_id);
...
<head>
    ...
    <title>Student Profile - Examify</title>
    <!-- Tailwind CSS -->
...
<body class="font-roboto bg-gray-900 text-white">
    ...
    <div class="max-w-7xl mx-auto py-12 px-6">
        ...
        <form action="student_profile.php" method="POST" class="bg-[#1a1a2e] p-8 rounded-lg shadow-lg">
            <div class="mb-4">
                <label for="name" class="block text-lg font-medium mb-2">Name</label>
                <input type="text" id="name" name="name" class="w-full p-3 rounded bg-gray-800 text-white" value="<?php echo htmlspecialchars($student['name']); ?>" required>
            </div>
            <div class="mb-4">
                <label for="email" class="block text-lg font-medium mb-2">Email</label>
                <input type="email" id="email" name="email" class="w-full p-3 rounded bg-gray-800 text-white" value="<?php echo htmlspecialchars($student['email']); ?>" required>
            </div>
            <div class="mb-4">
